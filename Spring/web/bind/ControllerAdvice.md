...
默认情况下，`@ControllerAdvice` 中的方法全局应用于所有控制器。使用选择器 `annotation()`、`basepackageclass()`和 `basePackages()` \(或其别名 `value()`\)来定义更精确的的目标 Controllers 子集。如果声明了多个选择器，或者应用了逻辑，这意味着所选控制器应该至少匹配一个选择器。注意，选择器检查是在运行时执行的，因此添加许多选择器可能会对性能产生负面影响，并增加复杂性。
